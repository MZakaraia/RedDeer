...
class RedDeerOptimization_Chaotic:
    ...
    def optimize(self):
        # Initialize population based chaiotic logistic mapping
        self.population = generate_initial_population(self.pop_size, self.dimensions\
...
def logistic_map_chaos(seed, r, size):
    chaos = np.zeros(size)
    x = seed
    for i in range(size):
        x = r * x * (1 - x)
...
def generate_initial_population(pop_size, dim, lower_bound, upper_bound):
    seed = 0.5  # Initial seed in (0,1)
    r = 3.99    # Chaotic parameter
    total_values = pop_size * dim
    chaos_vals = logistic_map_chaos(seed, r, total_values)
    
    # Scale chaotic values to [lower_bound, upper_bound]
    chaos_vals = lower_bound + (upper_bound - lower_bound) * chaos_vals
    population = chaos_vals.reshape((pop_size, dim))
    return population